...
from modules.nested_conv import NestDepthConv


CANDIDATES = {
    'zero': lambda C, stride, affine: Zero(stride=stride),
    'nested_conv': lambda C, stride, affine: NestSepConv(C, C, 3, stride, 1, affine=affine),
...
class NestSepConv(nn.Module):

    def __init__(self, C_in, C_out, kernel_size=3, stride=1, padding=1, affine=True):
        super(NestSepConv, self).__init__()
        self.act = nn.ReLU(inplace=False)
        self.depth_conv1 = NestDepthConv(C_in, kernel_size=kernel_size, stride=stride, padding=padding)
        self.point_conv1 = nn.Conv2d(C_in, C_in, kernel_size=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(C_in, affine=affine)

        self.depth_conv2 = NestDepthConv(C_in, kernel_size=kernel_size, stride=1, padding=padding)
        self.point_conv2 = nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(C_out, affine=affine)

    def forward(self, x, has_residual):
        x = self.act(x)
        x = self.depth_conv1(x, has_residual)
        x = self.point_conv1(x)
        x = self.bn1(x)
        x = self.act(x)
        x = self.depth_conv2(x, has_residual)
